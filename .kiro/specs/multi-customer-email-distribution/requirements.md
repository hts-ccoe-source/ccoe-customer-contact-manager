...
"CCOE Customer Contact Manager" is a multi-customer email distribution system that supports change management and other notification use cases across multiple customer AWS Organizations in a Managed Service Provider (MSP) environment. The system includes a complete web portal for change notifications, Lambda-based backend services, and a comprehensive Go CLI (CCOE Customer Contact Manager) that handles SES mailing list & topic management, SQS message processing, and Identity Center integration. The architecture supports ~30 customer organizations while maintaining proper isolation and security boundaries.

The intent is for the CCOE team to use the <https://change-management.ccoe.hearst.com> website (static html, css, and javascript hosted on a custom domain with a valid auto-renewing SSL Certificate) to write a notification once and be able to select which customer(s) it applies to.  The front-end creates a `json` object that is placed into an S3 bucket.  The S3 bucket has notifications enabled and when the frontend saves the `json` at a key prefix,  or gets a tag, one SQS queue for each customer gets a message.

The Go Lang backend has both a CLI mode of operation, and a Lambda handler connected directly to SQS.  The CLI mode is able to query information about the Organization as well as AWS IAM Identity Center, and then automatically provision SES List Management subscriptions based on if an Identity Center User has a particular type of IAM Role (such as 'security', 'cloudeng').  The CLI mode list management commands should run for every customer: event-drive or on a schedule.

The event from SQS executes the backend process as a Lambda which gets the corresponding `json` object from S#. Then business logic in the backend decides which SES List Topic the object maps to (currently, 'approval request' -> 'aws-approval', 'approved announcement' -> 'aws-announce', and 'completed' -> 'aws-announce' are valid message types, potentially others for enablement opportunities or PAS team communications) and then assumes a new IAM Role inside each customer's own SES-enabled AWS Organization member Account (`common-nonprod` currently for `htsnonprod`) and uses the customers own Simple Email Service List Management service to send templated emails to Contacts who are subscribed to the mapped topic.
